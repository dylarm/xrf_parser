...
_Applications: List[bytes] = [b"GLASSMAJOR", b"GeoExploration", b"GeoMining"]
_Methods: List[bytes] = [b"Oxide3phase", b"OxideConcentrates", b"GLASSMAJORS"]
...
FILE_FOOTERS: List[bytes] = [
    pack(FMT_FOOTER, 7, 34, 1028443341, 9, x, 5, 8) for x in [102, 124, 126]
]
APPLICATIONS: List[bytes] = [pack(string_fmt(s), *list(s)) for s in _Applications]
METHODS: List[bytes] = [pack(string_fmt(s), *list(s)) for s in _Methods]
